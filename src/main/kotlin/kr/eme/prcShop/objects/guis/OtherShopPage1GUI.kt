...
class OtherShopPage1GUI(player: Player) : GUI(player, "§f\\u340F\\u3419", 6) {
    override fun setFirstGUI() {
        val items = ShopItems.getShopItems("OtherShop", 1)
        for (item in items) {
    ...
    override fun InventoryClickEvent.clickEvent() {
        ...
        if (isLeftClick) {
            ...
            val shopItem = ShopItems.getShopItems("OtherShop", 1).find { it.slot == slot }
            val buyGUI = BillBuyGUI(player, clickedItem.clone(), this@OtherShopPage1GUI, shopItem)
            buyGUI.setFirstGUI()
            ...
            val shopItem = ShopItems.getShopItems("OtherShop", 1).find { it.slot == slot }
            val sellGUI = BillSellGUI(player, clickedItem.clone(), this@OtherShopPage1GUI, shopItem)
            sellGUI.setFirstGUI()
