...
class FarmingShopPage2GUI(player: Player) : GUI(player, "§f\\u340F\\u3416" ,6){
    ...
    override fun InventoryClickEvent.clickEvent() {
        ...
        if (isLeftClick) {
            ...
            val shopItem = ShopItems.getShopItems("FarmingShop", 2).find { it.slot == slot }
            val buyGUI = BillBuyGUI(player, clickedItem.clone(), this@FarmingShopPage2GUI, shopItem)
            buyGUI.setFirstGUI()
            ...
            val shopItem = ShopItems.getShopItems("FarmingShop", 2).find { it.slot == slot }
            val sellGUI = BillSellGUI(player, clickedItem.clone(), this@FarmingShopPage2GUI, shopItem)
            sellGUI.setFirstGUI()
