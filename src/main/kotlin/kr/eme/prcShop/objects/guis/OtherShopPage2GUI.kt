        ...
        if (isLeftClick) {
            when (ExchangeUtil.tryTrade(player, "OtherShop", 2, itemDisplayName)) {
                ...
                is ExchangeUtil.TradeResult.NotTradeItem -> {
                    ...
                    val shopItem = ShopItems.getShopItems("OtherShop", 2).find { it.slot == slot }
            val buyGUI = BillBuyGUI(player, clickedItem.clone(), this@OtherShopPage2GUI, shopItem)
                    buyGUI.setFirstGUI()
            ...
            val shopItem = ShopItems.getShopItems("OtherShop", 2).find { it.slot == slot }
            val sellGUI = BillSellGUI(player, clickedItem.clone(), this@OtherShopPage2GUI, shopItem)
            sellGUI.setFirstGUI()
