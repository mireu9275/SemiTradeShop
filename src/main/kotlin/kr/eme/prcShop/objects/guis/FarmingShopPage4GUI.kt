...
class FarmingShopPage4GUI(player: Player) : GUI(player, "§f\\u340F\\u3418" ,6){
    ...
    override fun InventoryClickEvent.clickEvent() {
        ...
        if (isLeftClick) {
            ...
            val shopItem = ShopItems.getShopItems("FarmingShop", 4).find { it.slot == slot }
            val buyGUI = BillBuyGUI(player, clickedItem.clone(), this@FarmingShopPage4GUI, shopItem)
            buyGUI.setFirstGUI()
            ...
            val shopItem = ShopItems.getShopItems("FarmingShop", 4).find { it.slot == slot }
            val sellGUI = BillSellGUI(player, clickedItem.clone(), this@FarmingShopPage4GUI, shopItem)
            sellGUI.setFirstGUI()
