...
class FarmingShopPage1GUI(player: Player) : GUI(player, "§f\\u340F\\u3415" ,6){
    ...
    override fun InventoryClickEvent.clickEvent() {
        ...
        if (isLeftClick) {
            ...
            val shopItem = ShopItems.getShopItems("FarmingShop", 1).find { it.slot == slot }
            val buyGUI = BillBuyGUI(player, clickedItem.clone(), this@FarmingShopPage1GUI, shopItem)
            buyGUI.setFirstGUI()
            ...
            val shopItem = ShopItems.getShopItems("FarmingShop", 1).find { it.slot == slot }
            val sellGUI = BillSellGUI(player, clickedItem.clone(), this@FarmingShopPage1GUI, shopItem)
            sellGUI.setFirstGUI()
