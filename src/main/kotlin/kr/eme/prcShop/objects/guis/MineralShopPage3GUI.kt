...
class MineralShopPage3GUI(player: Player) : GUI(player, "§f\\u340F\\u3414", 6) {
    ...
    override fun InventoryClickEvent.clickEvent() {
        ...
        if (isLeftClick) {
            ...
            val shopItem = ShopItems.getShopItems("MineralShop", 3).find { it.slot == slot }
            val buyGUI = BillBuyGUI(player, clickedItem.clone(), this@MineralShopPage3GUI, shopItem)
            buyGUI.setFirstGUI()
            ...
            val shopItem = ShopItems.getShopItems("MineralShop", 3).find { it.slot == slot }
            val sellGUI = BillSellGUI(player, clickedItem.clone(), this@MineralShopPage3GUI, shopItem)
            sellGUI.setFirstGUI()
