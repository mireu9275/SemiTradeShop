...
class OtherShopPage3GUI(player: Player) : GUI(player, "§f\\u340F\\u3421", 6) {
    override fun setFirstGUI() {
        val items = ShopItems.getShopItems("OtherShop", 3)
        for (item in items) {
    ...
    override fun InventoryClickEvent.clickEvent() {
        ...
        when (itemDisplayName) {
            ...
            "§f오른쪽으로 이동" -> {
                val otherShopPage4GUI = OtherShopPage4GUI(player)
                otherShopPage4GUI.setFirstGUI()
                GUIManager.setGUI(player.uniqueId, otherShopPage4GUI)
                otherShopPage4GUI.open()
                SoundUtil.click(player)
                return
            }
        }
        ...
        if (isLeftClick) {
            when (ExchangeUtil.tryTrade(player, "OtherShop", 3, itemDisplayName)) {
                ...
                is ExchangeUtil.TradeResult.NotTradeItem -> {
                    ...
                    val shopItem = ShopItems.getShopItems("OtherShop", 3).find { it.slot == slot }
            val buyGUI = BillBuyGUI(player, clickedItem.clone(), this@OtherShopPage3GUI, shopItem)
                    buyGUI.setFirstGUI()
            ...
            val shopItem = ShopItems.getShopItems("OtherShop", 3).find { it.slot == slot }
            val sellGUI = BillSellGUI(player, clickedItem.clone(), this@OtherShopPage3GUI, shopItem)
            sellGUI.setFirstGUI()
