        ...
        if (isLeftClick) {
            when (ExchangeUtil.tryTrade(player, "OtherShop", 4, itemDisplayName)) {
                ...
                is ExchangeUtil.TradeResult.NotTradeItem -> {
                    ...
                    val shopItem = ShopItems.getShopItems("OtherShop", 4).find { it.slot == slot }
            val buyGUI = BillBuyGUI(player, clickedItem.clone(), this@OtherShopPage4GUI, shopItem)
                    buyGUI.setFirstGUI()
            ...
            val shopItem = ShopItems.getShopItems("OtherShop", 4).find { it.slot == slot }
            val sellGUI = BillSellGUI(player, clickedItem.clone(), this@OtherShopPage4GUI, shopItem)
            sellGUI.setFirstGUI()
